...
sudo git clone https://github.com/intel-restricted/frameworks.validation.maestro.maestro.git /root/maestro
sudo git clone https://github.com/intel-sandbox/pcie_automation.git /root/pcie_automation
...
cd install
./perspec_svos_install.sh <svos_perspec_ver>.tgz
#Maestro Environment setup
export $MAESTRO_REPO_PATH=/root/maestro
export $PCIE_AUTOMATION_REPO_PATH=/root/pcie_automation
...
export CADENCE_INSTALL_ROOT=/opt/cad
mkdir -p /root/pcie_automation
mkdir -p /root/pcie_automation/execution
'''
...
class PerspecTestLine:
  ...
  def get_ps_cmd(self):
    self.ps_cmd = 'perspec generate -define PCIE_NUM_CTRLS {0} -define PCIE_TEST_CTRL {1} -define CONTENT_REGRESSION_FLAG {2} -define PCIE_TEST_CTRLS {{{3}}} -f {4} -sln {5} -top_action {6} -generate_test_table {7} -target_dir {8}'.format(self.pcie_num_ctrls,self.pcie_test_ctrls, self.content_regression_flag, self.pcie_test_ctrls,
    ...
    self.ps_cmd2 = '$MAESTRO_REPO_PATH/build/content/linux/skl/root/pcie_automation/execution/{0}/{1}'.format(self.top_action, self.top_action)
    return [self.ps_cmd,self.ps_cmd1,self.ps_cmd2]
  def get_nga_ps_cmd(self):
      self.nga_maestro_platform_config = "python -m evtar.lotus.workflows.tornado.ux.TornadoPlatformConfigWorkflow"
      self.nga_maestro_download_modules = "download_modules --maestro-version integration"
      self.nga_maestro_container_genbuild = "python -m evtar.lotus.workflows.tornado.ux.TornadoGenerationWorkflowContainer --command ""perspec generate -define PCIE_NUM_CTRLS {0} -define PCIE_TEST_CTRL {1} -define CONTENT_REGRESSION_FLAG {2} -define PCIE_TEST_CTRLS {{{3}}} -f {4} -sln {5} -top_action {6} -generate_test_table {7}"" --maestro-version ""integration"" --nga-env ""Prod""".format(self.pcie_num_ctrls,self.pcie_test_ctrls, self.content_regression_flag, self.pcie_test_ctrls,
                                                                                                                                                                                                                                            self.f, self.sln, self.top_action, self.generate_test_table, self.top_action)
      self.nga_maestro_container_exec = "python -m evtar.lotus.workflows.tornado.ux.TornadoExecutionWorkflowContainer --nga-env ""Prod"""
